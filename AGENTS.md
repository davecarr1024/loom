...
plan has accepted NOT, AND, OR, constant bit, initialized DFF, and their typed
observation/edge harness. The next group begins with gate-composed XOR and mux.
Read the component contracts under `docs/` for those APIs. Do not resume the
shelved arithmetic prototype.
...
`bazel run //:constant_bit_demo`, `bazel run //:d_flip_flop_demo`, and
