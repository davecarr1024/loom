...
two-source selected bus and two-register bank are accepted. Half adder, full
adder, and four-bit ripple adder are accepted group-E components; increment
and equality components are next. Read the relevant contract under `docs/`
before extending a component.
...
`bazel run //:full_adder_demo`, `bazel run //:ripple_adder_demo`, and
