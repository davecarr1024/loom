...
two-source selected bus and two-register bank are accepted; arithmetic is
next. Read the relevant contract under `docs/` before extending a component.
...
`bazel run //:register_bank_2_demo`, and `bazel run //:transfer` to run the
